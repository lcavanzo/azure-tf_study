...
resource "azurerm_linux_virtual_machine" "web_linuxvm" {
  ...
  name = "${local.resource_name_prefix}-web_linuxvm-${count.index}"
  computer_name = "web-linux-vm-${count.index}" # Hostname of the vm
  ...
  admin_ssh_key {
    ...
    public_key = file("~/git/ssh-keys/terraform-azure.pub")
  }
